...
# Plot the smoothed SNR DB data with a taller figure
fig, ax = plt.subplots(figsize=(10, 7))
line, = ax.plot(data_belpha_new['DateTime'], data_belpha_new['SNR_DB_Smoothed'], color='black',
                label='Smoothed SNR DB (Moving Average)', zorder=1)
...
plt.legend(handles=[night_patch, ax.lines[0]], loc='lower left')

# Set x-axis limits to just before and after the data range
ax.set_xlim([start_date, end_date])

# Set y-axis limits to ensure the -8 line is visible
ax.set_ylim(bottom=-10)  # Adjust if needed
...
plt.ylabel('SNR DB')
plt.title('Brockwier Woods 51.709262, -2663536')
